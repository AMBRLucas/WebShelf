...
public class LoanService {
    ...
    public Loan ReturnBookByBookId(String id){
        Loan activeLoan = this.loanRepository.findActiveLoanByBookId(id).orElseThrow();
        Client client = this.clientRepository.findById(activeLoan.getClient().getId()).orElseThrow();
        Book book = this.bookRepository.findById(activeLoan.getBook().getId()).orElseThrow();
    ...
    public List<Map<String, Object>> getLoanByDay(int month, int year, String libraryId){
        
        return loanRepository.findLoansByDayAndLibraryId(month, year, libraryId);
    }
}
